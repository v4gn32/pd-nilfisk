...
import { FileText, FileBadge, FileSpreadsheet, FileWarning, Users, Calendar } from 'lucide-react';
                ...
                {documents.slice(0, 5).map((doc) => (
                  <div key={doc.id} className="flex items-center p-3 rounded-md hover:bg-gray-50">
                    <div className="p-2 rounded-md bg-gray-100 mr-4">
                      {doc.type === 'HOLERITE' && <FileText className="text-blue-500\" size={20} />}
                      {doc.type === 'FERIAS' && <FileBadge className="text-green-500\" size={20} />}
                      {doc.type === 'COMISSAO' && <FileSpreadsheet className="text-purple-500\" size={20} />}
                      {doc.type === 'INFORME_RENDIMENTO' && <FileWarning className="text-orange-500\" size={20} />}
                    </div>
              ...
              {isAdmin && (
                <div className="pt-4">
                  ...
                  <div className="flex items-center p-3 bg-[#28313F]/5 rounded-md">
                    ...
                    <span className="text-gray-600">Gerenciar Usuários e Documentos</span>
                  </div>
                </div>
              )}
            </div>
